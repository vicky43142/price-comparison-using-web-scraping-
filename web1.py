...
def flipkart(name = ""):
    try:
        ...
        # flipkart_name = soup.select('KzDlHZ')[0].getText().strip()  ### New Class For Product Name
        flipkart_name = soup.find('div', attrs={"class": "KzDlHZ"}).text
        print('Product Name : ' +flipkart_name)
        flipkart_name = flipkart_name.upper()
        if name.upper() in flipkart_name:
            flipkart_price = soup.find('div', attrs={"class": "Nx9bqj _4b5DiR"}).text  ### New Class For Product Price
            flipkart_name = soup.find('div', attrs={"class": "KzDlHZ"}).text
            return_flipkart = f"{flipkart_name}\nPrice : {flipkart_price}\n"

            return return_flipkart
        else:

            return_flipkart='           Product Not Found'
        return return_flipkart
    except:
...
window = Tk()
window.wm_title("Price comparison extinction")
